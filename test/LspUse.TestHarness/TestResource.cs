...
public static class TestResource
{
    ...
    /// </summary>
    public static Uri ApplicationService =>
        new(GetSourcePath("LspUse.Application/ApplicationService.cs"));

    /// <summary>
    /// Gets the URI to FindReferencesResult.cs source file.
    /// </summary>
    public static Uri FindReferencesResult =>
        new(GetSourcePath("LspUse.Application/FindReferencesResult.cs"));

    /// <summary>
    /// Gets the URI to FileLoggerProvider.cs source file.
    /// </summary>
    public static Uri FileLoggerProvider =>
        new(GetSourcePath("LspUse.McpServer/FileLoggerProvider.cs"));

    /// <summary>
    /// Gets the URI to FindReferencesTool.cs source file.
    /// </summary>
    public static Uri FindReferencesTool =>
        new(GetSourcePath("LspUse.McpServer/Tools/FindReferencesTool.cs"));

    /// <summary>
    /// Gets the URI to IApplicationService.cs source file.
    /// </summary>
    public static Uri IApplicationService =>
        new(GetSourcePath("LspUse.Application/IApplicationService.cs"));
    ...
    private static string GetTestSourcePath(string fileName) =>
        Path.Combine(RepositoryRoot, "test", "LspUse.TestHarness", "TestSources", fileName);
    ...
    private static string GetSourcePath(string relativePath) =>
        Path.Combine(RepositoryRoot, "src", relativePath);
    ...
    private static string FindRepositoryRoot([CallerFilePath] string? callerFilePath = null)
    {
        if (callerFilePath == null)
        {
            throw new InvalidOperationException(
                "Unable to determine caller file path for repository root detection");
        }
        ...
        while (directory != null)
        {
            ...
            if (directory.GetDirectories(".git").Any() || directory.GetFiles("*.sln").Any() ||
